...
def test_exec_guard_allows_configured_extra_root(tmp_path) -> None:
    ...
    assert tool._guard_command("git status", str(repo)) is None
    assert tool._guard_command(f"cat {repo / 'README.md'}", str(repo)) is None


def test_exec_guard_blocks_working_dir_outside_allowed_roots(tmp_path) -> None:
    workspace = tmp_path / "ws"
    workspace.mkdir()
    repo = tmp_path / "repo"
    repo.mkdir()
    ...
    error = tool._guard_command("git status", str(outside))
